...
public:
	...
	struct ElapsedTick {
		...
		template<typename D, typename T>
		T GetTick() const requires (is_number<T>) {
			return std::chrono::duration_cast<std::chrono::duration<T, typename D::period>>(m_elapsed).count();
		}
